...
const Calendar: React.FC<CalendarProps> = ({
  ...
  const shiftDate = useCallback((date: string, days: number): string => {
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      const baseDateForNavigation =
        state.focusedDate ?? state.selectedDate ?? today;

      if (event.key === "ArrowRight" || event.key === "ArrowDown") {
        event.preventDefault();
        const nextDate = shiftDate(
          baseDateForNavigation,
          event.key === "ArrowRight" ? 1 : 7,
        );
        const nextMonthOffset = getNumberOfMonthsBetweenDates(
          startMonth,
          nextDate,
        );
        ...
        event.preventDefault();
        const prevDate = shiftDate(
          baseDateForNavigation,
          event.key === "ArrowLeft" ? -1 : -7,
        );
        const prevMonthOffset = getNumberOfMonthsBetweenDates(
          startMonth,
          prevDate,
        );
