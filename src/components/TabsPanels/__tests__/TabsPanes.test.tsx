...
const panel3 = { id: "tab3", content: <div>Pane 3 Market</div> };
...
describe("TabsPanels", () => {
  test("should return the complete component template (snapshot)", () => {
    expect(
      render(
        <TabsPanels activePanel={panel1.id} panels={panels} className="mix" />,
      ),
    ).toMatchSnapshot();
  });
  ...
  test("should display only the active pane content (snapshot)", () => {
    expect(
      render(<TabsPanels activePanel={panel1.id} panels={panels} />),
    ).toMatchSnapshot();
  });
